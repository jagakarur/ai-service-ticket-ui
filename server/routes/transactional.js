...
router.post('/doExplainabilityStage1', function (req, res, next) {
  var data = req.body.data ? req.body.data : '';
  var config = {
    method: 'post',
  ...
  axios(config)
    .then(function (response) {
      let resultObj = response.data.word_attributions ? response.data.word_attributions.slice(1, -1).map((element) => {
        return {
          key: element[0],
          value: element[1]
        }
      ...
      //console.log(JSON.stringify(response.data));
      res.json({
        message: response.data.output_label,
        word_attribute: resultObj
      });
    })
    .catch(function (error) {
      console.log(error);
